...
#define DISPLAY_UPDATE_PERIOD_MS 1u
#define LED_UPDATE_PERIOD_MS 50u
#define SERIAL_OUTPUT_PERIOD_MS SAMPLE_PERIOD_MS
...
#define TIM2_MILLISECOND_TICKS      ((1000u + TIM2_TICK_PERIOD_US - 1u) / TIM2_TICK_PERIOD_US)
#define SAMPLE_PERIOD_TICKS         ((SAMPLE_PERIOD_MS * 1000u) / TIM2_TICK_PERIOD_US)
#define DISPLAY_UPDATE_PERIOD_TICKS ((DISPLAY_UPDATE_PERIOD_MS * 1000u) / TIM2_TICK_PERIOD_US)
...
#define OLED_I2C_DELAY_CYCLES      100u
#define OLED_STATUS_Y              9u
#define OLED_STATUS_SCALE          2u
#define OLED_STATUS_PAGE_START     (OLED_STATUS_Y / 8u)
#define OLED_STATUS_PAGE_COUNT     (((OLED_STATUS_Y + (OLED_FONT_HEIGHT * OLED_STATUS_SCALE) + 7u) / 8u) - OLED_STATUS_PAGE_START)
#define SERIAL_TX_BUFFER_SIZE      256u
#define SERIAL_RISK_LINE_MAX       64u
...
static volatile uint32_t tim2_display_period_elapsed = 0u;
static volatile uint32_t tim2_display_tick_count = 0u;
static volatile uint32_t tim2_millisecond_count = 0u;
static volatile uint32_t tim2_millisecond_tick_count = 0u;
static volatile uint32_t buzzer_enabled = 0u;
static volatile uint32_t buzzer_output_high = 0u;
static volatile uint32_t usart2_tx_head = 0u;
static volatile uint32_t usart2_tx_tail = 0u;
static uint8_t usart2_tx_buffer[SERIAL_TX_BUFFER_SIZE];
...
{
    const uint32_t start = tim2_millisecond_count;

    while ((uint32_t)(tim2_millisecond_count - start) < ms) {
        __asm volatile ("wfi");
    }
}

static uint32_t median_filter_update(MedianFilter *filter, uint16_t sample)
{
    uint16_t sorted_samples[FILTER_SAMPLE_COUNT];
    ...
    for (uint32_t i = 0u; i < filter->count; ++i) {
        sorted_samples[i] = filter->samples[i];
    }

    for (uint32_t i = 1u; i < filter->count; ++i) {
        uint16_t value = sorted_samples[i];
        uint32_t j = i;
    ...
    if ((filter->count & 1u) == 0u) {
        return ((uint32_t)sorted_samples[(filter->count / 2u) - 1u] +
                sorted_samples[filter->count / 2u]) / 2u;
    }

    return sorted_samples[filter->count / 2u];
}
...
static void usart2_init(void)
{
    ...
    NVIC_ISER1 = (1u << (NVIC_IRQ_USART2 - 32u));
}

static uint32_t usart2_next_tx_index(uint32_t index)
{
    index++;
...
static uint32_t usart2_tx_free_count(void)
{
    uint32_t head;
    uint32_t tail;
    uint32_t used;
    ...
    if (head >= tail) {
        ...
        used = SERIAL_TX_BUFFER_SIZE - (tail - head);
    }

    return (SERIAL_TX_BUFFER_SIZE - 1u) - used;
}

static uint32_t usart2_write_char(char ch)
{
    uint32_t queued = 0u;

    __asm volatile ("cpsid i" ::: "memory");
    const uint32_t next_head = usart2_next_tx_index(usart2_tx_head);
    if (next_head != usart2_tx_tail) {
        usart2_tx_buffer[usart2_tx_head] = (uint8_t)ch;
...
static uint32_t usart2_write_string(const char *text)
{
    while (*text != '\0') {
        if (usart2_write_char(*text) == 0u) {
            return 0u;
        }
        text++;
    }
...
static uint32_t usart2_write_u32(uint32_t value)
{
    char digits[10];
    uint32_t count = 0u;

    if (value == 0u) {
        return usart2_write_char('0');
    }
    ...
    while (count > 0u) {
        if (usart2_write_char(digits[--count]) == 0u) {
            return 0u;
        }
    }
...
{
    if ((USART2_SR & USART_SR_TXE) != 0u) {
        if (usart2_tx_tail != usart2_tx_head) {
            USART2_DR = usart2_tx_buffer[usart2_tx_tail];
            usart2_tx_tail = usart2_next_tx_index(usart2_tx_tail);
        } else {
            USART2_CR1 &= ~USART_CR1_TXEIE;
        }
    }
...
{
    for (volatile uint32_t i = 0u; i < OLED_I2C_DELAY_CYCLES; ++i) {
        __asm volatile ("nop");
    }
}
...
static void oled_flush_pages(uint32_t first_page, uint32_t page_count)
{
    uint8_t commands[6];

    if ((first_page >= OLED_PAGE_COUNT) || (page_count == 0u)) {
        return;
    }
    if ((first_page + page_count) > OLED_PAGE_COUNT) {
    ...
    commands[3] = 0x22u;
    commands[4] = (uint8_t)first_page;
    commands[5] = (uint8_t)(first_page + page_count - 1u);
    oled_write_commands(commands, 6u);
    oled_write_data(&oled_buffer[first_page * OLED_WIDTH],
                    OLED_WIDTH * page_count);
}

static void oled_flush(void)
{
    oled_flush_pages(0u, OLED_PAGE_COUNT);
}

static void oled_clear_status_buffer(void)
{
    const uint32_t first = OLED_STATUS_PAGE_START * OLED_WIDTH;
    const uint32_t count = OLED_STATUS_PAGE_COUNT * OLED_WIDTH;

    for (uint32_t i = 0u; i < count; ++i) {
        oled_buffer[first + i] = 0u;
    }
}
...
{
    oled_clear_status_buffer();
    oled_draw_text_centered(status, OLED_STATUS_Y, OLED_STATUS_SCALE);
    oled_flush_pages(OLED_STATUS_PAGE_START, OLED_STATUS_PAGE_COUNT);
}

static void oled_show_status_full(const char *status)
{
    oled_clear_buffer();
    oled_draw_text_centered(status, OLED_STATUS_Y, OLED_STATUS_SCALE);
    oled_flush();
...
static uint32_t tim2_take_display_tick(void)
{
    uint32_t tick_available = 0u;

    __asm volatile ("cpsid i" ::: "memory");
    if (tim2_display_period_elapsed != 0u) {
        tim2_display_period_elapsed--;
        tick_available = 1u;
    }
    __asm volatile ("cpsie i" ::: "memory");
...
static void tim2_init(void)
{
    ...
    tim2_display_period_elapsed = 0u;
    tim2_display_tick_count = 0u;
    tim2_millisecond_count = 0u;
    tim2_millisecond_tick_count = 0u;
    TIM2_CR1 = 0u;
...
static void tim2_clear_pending_work_ticks(void)
{
    __asm volatile ("cpsid i" ::: "memory");
    tim2_period_elapsed = 0u;
    tim2_sample_tick_count = 0u;
...
{
    if (usart2_tx_free_count() < SERIAL_RISK_LINE_MAX) {
        return;
    }

    (void)usart2_write_string("L=");
    (void)usart2_write_u32(light_level);
    (void)usart2_write_string(" R=");
    (void)usart2_write_u32(resistance_level);
    (void)usart2_write_string(" C=");
    (void)usart2_write_u32(risk_level);
    (void)usart2_write_string(" STATUS=");
    (void)usart2_write_string(risk_text);
    (void)usart2_write_string(" OLED=");
    (void)usart2_write_string(oled_detect_text);
    (void)usart2_write_string("\r\n");
}
...
void TIM2_IRQHandler(void)
{
    if ((TIM2_SR & TIM_SR_UIF) != 0u) {
        ...
        tim2_millisecond_tick_count++;
        if (tim2_millisecond_tick_count >= TIM2_MILLISECOND_TICKS) {
            tim2_millisecond_tick_count = 0u;
            if (tim2_millisecond_count < 0xFFFFFFFFu) {
                tim2_millisecond_count++;
            }
        }
        ...
        tim2_display_tick_count++;
        if (tim2_display_tick_count >= DISPLAY_UPDATE_PERIOD_TICKS) {
            tim2_display_tick_count = 0u;
            if (tim2_display_period_elapsed < 0xFFFFFFFFu) {
                tim2_display_period_elapsed++;
            }
        }
    }
...
int main(void)
{
    MedianFilter light_filter = {0u};
    MedianFilter resistance_filter = {0u};
    uint32_t led_update_tick_count = 0u;
    uint32_t serial_output_tick_count = 0u;
    uint32_t oled_status_update_pending = 0u;
    const char *oled_pending_status = "SAFE";
    char last_status_initial = 'S';
    ...
    for (;;) {
        ...
        uint32_t resistance_level;
        uint32_t filtered_risk_level;
        uint32_t risk_level;
        const char *risk_text;
        uint32_t status_changed;

        if (tim2_take_display_tick() != 0u) {
            if (oled_status_update_pending != 0u) {
                oled_show_status(oled_pending_status);
                oled_status_update_pending = 0u;
            }
        }
        ...
        light_level = median_filter_update(&light_filter, light_raw);
        resistance_level = median_filter_update(&resistance_filter, resistance_raw);
        filtered_risk_level = calculate_risk_percent(light_level, resistance_level);
        risk_level = filtered_risk_level;
        ...
        if (status_changed != 0u) {
            ...
            oled_pending_status = risk_text;
            oled_status_update_pending = 1u;
