...
int main(void)
{
    ...
    usart2_write_string("STM32F103 A1 CDS ADC ready\r\n");

    for (;;) {
        uint32_t adc_raw = adc1_read_a1();
        uint32_t millivolts = (adc_raw * VREF_MILLIVOLTS) / ADC_MAX_VALUE;

        usart2_write_string("A1 ADC: ");
