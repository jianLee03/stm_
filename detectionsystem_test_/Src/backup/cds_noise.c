...
int main(void)
{
    uint16_t adc_samples[ADC_AVERAGE_COUNT] = {0u};
    uint32_t adc_sum = 0u;
    ...
    usart2_write_string("STM32F103 A1 CDS timer average ready\r\n");
    ...
    for (;;) {
        ...
        if (sample_count < ADC_AVERAGE_COUNT) {
            adc_samples[sample_index] = (uint16_t)adc_raw;
            adc_sum += adc_raw;
            sample_count++;
        } else {
        ...
        sample_index++;
        if (sample_index >= ADC_AVERAGE_COUNT) {
            sample_index = 0u;
        }
