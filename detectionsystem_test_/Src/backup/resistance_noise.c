...
static uint16_t adc1_read_a0_average(void)
{
    uint32_t sum = 0u;

    for (uint32_t i = 0u; i < ADC_AVERAGE_COUNT; ++i) {
        sum += adc1_read_a0();
        delay_ms(2u);
    }
...
int main(void)
{
    ...
    usart2_write_string("STM32F103 A0 ADC average ready\r\n");

    for (;;) {
        uint32_t adc_average = adc1_read_a0_average();
